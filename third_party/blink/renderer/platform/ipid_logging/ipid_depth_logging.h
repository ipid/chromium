...
using format_type_converter_t =
    std::conditional_t<std::is_same_v<std::remove_cvref_t<T>, String> ||
...
/**
  智能值转换函数，能够自动处理：
  1. blink::String → std::string (通过 .Utf8())
...
class PLATFORM_EXPORT IpidDepthLog {
 public:
  ...
  // 打印日志。使用 std::format 语法，并且额外支持 blink::String 和具有
  // ToString() 方法的对象；日志会自动使用本类的语法来打印。
