...
LayoutUnit ResolveInlineLengthInternal(
  ...
  switch (length.GetType()) {
    ...
    case Length::kStretch: {
      ...
      if (margins_to_subtract > LayoutUnit()) {
        ipid_depth_log.FPrint(
            "由于存在 margin，需要从可用空间中减掉 {}px 的值，剩余的可用空间为 "
            "{}",
            margins_to_subtract, (available_size - margins_to_subtract));
      }

      LayoutUnit ret = std::max(border_padding.InlineSum(),
                                available_size - margins_to_subtract);
      if (border_padding.InlineSum() > available_size - margins_to_subtract) {
        ipid_depth_log.FPrint(
            "border + padding 的值： {}px，大于可用空间，故宽度 {} 的最终 "
            "resolve 结果为 {}。",
            border_padding.InlineSum(), length, ret);
      } else {
        ipid_depth_log.FPrint("宽度 {} 的最终 resolve 结果为 {}。", length,
                              ret);
      }
  ...
  switch (length.GetType()) {
    ...
    case Length::kStretch: {
      ...
      LayoutUnit ret = std::max(border_padding.BlockSum(),
                                available_size - margins_to_subtract);
      if (border_padding.BlockSum() > available_size - margins_to_subtract) {
        ipid_depth_log.FPrint(
            "border + padding 的值： {}px，大于可用空间，故高度 {} 的最终 "
            "解析结果为 {}。",
            border_padding.BlockSum(), length, ret);
      } else {
        ipid_depth_log.FPrint("高度 {} 的最终解析结果为 {}。", length, ret);
      }
